...
const ChartConfig = {
  AXIS_TYPE: 'number',
  AREA_TYPE: 'monotone',
  DOMAIN_AUTO: ['dataMin', 'dataMax'],
  INTERVAL_PRESERVE: 'preserveStartEnd',
  Y_AXIS_ID: 'angle'
};
...
const COMMON_AREA_PROPS = {
  yAxisId: ChartConfig.Y_AXIS_ID,
  type: ChartConfig.AREA_TYPE,
...
const KneeAreaChart = ({
  ...
  const toggleKneeVisibility = (kneeKey) => {
  ...
  const getTimestamp = (leftKnee, rightKnee) => {
    if (useSensorTimestamps) {
  ...
  const formatYAxis = (value) => `${value}${Labels.ANGLE_UNIT}`;

  /** Renders visibility toggle button for knee data series */
  const renderToggleButton = (config) => {
    const isVisible = kneeVisibility[config.key];
    return (
        <button
            key={config.key}
        ...
        <span className={CssClasses.FLEX_ITEMS_GAP}>
          {config.label}
          {isVisible && <Check className={UiSizing.ICON_SIZE} />}
        </span>
        </button>
    );
  };
  ...
  const renderKneeAreas = (config) => {
    if (!kneeVisibility[config.key]) return null;
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length > 0) {
      ...
      const formatAngle = (value) => value.toFixed(1); // 1 decimal place

      const renderTooltipEntry = (config) => {
  ...
  return (
      <div className={CssClasses.CHART_CONTAINER}>
        ...
        <ResponsiveContainer width={UiSizing.CONTAINER_WIDTH} height={UiSizing.CHART_HEIGHT}>
          <ComposedChart
              data={data}
          ...
          >
            ...
            <XAxis
                ...
                type={ChartConfig.AXIS_TYPE}
                domain={ChartConfig.DOMAIN_AUTO}
                tickFormatter={(time) => {
                  return new Date(time).toLocaleTimeString([], TimeFormat.OPTIONS);
                }}
                interval={ChartConfig.INTERVAL_PRESERVE}
            ...
            <ReferenceLine
                ...
                strokeDasharray={CHART_LAYOUT.GRID_DASH_ARRAY}
                label={{ value: Labels.STRAIGHT_REFERENCE, position: ChartPositions.REFERENCE_LABEL }}
            />
