...
export class OptimizedBinaryProtocol {
    ...
    static serializeMotionData(deviceName: string, quaternion: { w: number; x: number; y: number; z: number }): ArrayBuffer {
        ...
        // Timestamp (4 bytes) - use Date.now() for Node.js compatibility
    ...
    /**
     * Performance comparison utility - Node.js compatible
     */
    static benchmarkSerialization(iterations: number = 10000): { binary: number; json: number; improvement: string } {
        ...
        // Use hrtime for high precision timing in Node.js
        const hrtime = process.hrtime;

        // Test binary serialization
        const binaryStart = hrtime();
        for (let i = 0; i < iterations; i++) {
            this.serializeMotionData(testData.deviceName, testData.quaternion);
        }
        const binaryEnd = hrtime(binaryStart);
        const binaryTime = binaryEnd[0] * 1000 + binaryEnd[1] / 1000000; // Convert to milliseconds
        ...
        const jsonStart = hrtime();
        for (let i = 0; i < iterations; i++) {
            JSON.stringify(jsonData);
        }
        const jsonEnd = hrtime(jsonStart);
        const jsonTime = jsonEnd[0] * 1000 + jsonEnd[1] / 1000000; // Convert to milliseconds
